...
void Measurer::callbackPose(const geometry_msgs::Pose& msg)
{
  ...
  last_position[0] = msg.position.x; last_position[1] = msg.position.y; last_position[2] = msg.position.z;
  ...
  if (current_waypoint< waypoints.size() and waypoints[current_waypoint].distance(tf::Vector3(msg.position.x,msg.position.y,msg.position.z))<2.0)
  {
    ...
    std::cout << "Distance: " << distance <<" Ncollisions: " << collisions << " Time: " << (ros::Time::now()-initial_time).toSec()
...
void Measurer::callbackForce(const geometry_msgs::WrenchStamped& msg)
{
  ...
  {
    if (msg.wrench.force.x !=0  or msg.wrench.force.y != 0 or msg.wrench.force.z != 0 or msg.wrench.torque.x != 0 or msg.wrench.torque.y != 0 or msg.wrench.torque.z != 0)
    {
      collisions++;
      last_collision=ros::Time::now();
      std::cout << "collisions: " << collisions << "  " << msg.wrench.force.x << "  " << msg.wrench.force.y << "  " << msg.wrench.force.z
...
void Measurer::callbackThrusters(const nav_msgs::Odometry& msg)
{
  if (started)
  {
    ...
    last_thrusters [0] = msg.twist.twist.linear.x; last_thrusters [1] = msg.twist.twist.linear.y;
    last_thrusters [2] = msg.twist.twist.linear.z; last_thrusters [3] = msg.twist.twist.angular.z;
}
...
Measurer::Measurer()
{
  ...
  last_position[0] = 0; last_position[1] = 0; last_position[2] = 0;

  output.open("measurer_hrov_data.txt");
  if (output.is_open())
